@include('site.layouts.header')

<!-- Start Breadcrumb -->
	<div id="mu-breadcrumb">
		<div class="container">
			<div class="row">
				<div class="col-md-12">
					<nav aria-label="breadcrumb" role="navigation">
					  <ol class="breadcrumb mu-breadcrumb">
					    <li class="breadcrumb-item"><a href="{{ route('site.home') }}">Home</a></li>
					    <li class="breadcrumb-item active" aria-current="page">Parceiros</li>
					  </ol>
					</nav>
				</div>
			</div>
		</div>
	</div>
<!-- End Breadcrumb -->

	<!-- Start main content -->
	<main>
		
		<!-- Start Portfolio -->
		<section id="mu-portfolio">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<div class="mu-portfolio-area">
							<!-- Title -->
							<div class="row">
								<div class="col-md-12">
									<div class="mu-title">
										<h2>Patrocinadores</h2>
										<p>Enduro do Cerrado edição 2023.</p>
									</div>
								</div>
							</div>

								<!-- Start Team Content -->
								<div class="row">
									<div class="col-md-12">
										<div class="mu-team-content">
											<div class="row">
												<!-- start single item -->
												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/1.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://www.facebook.com/pages/Grauna-Motos-e-Motores/220730471610913" target="_blank"><i class="fa fa-facebook"></i></a></li>
																<li><a href="https://wa.me/5599984050445" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="https://www.instagram.com/graunamotos_carolinama/" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>
												<!-- End single item -->
												
												<!-- start single item -->
												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/2.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://www.facebook.com/cabral.forros.9" target="_blank"><i class="fa fa-facebook"></i></a></li>
																<li><a href="https://wa.me/5599991278230" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="#"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/3.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://www.facebook.com/groups/800076840703974" target="_blank"><i class="fa fa-facebook"></i></a></li>
																<li><a href="https://contate.me/endurodocerrado" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="https://www.instagram.com/saladoempreendedorcarolina/" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/4.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://www.facebook.com/profile.php?id=100080684388753" target="_blank"><i class="fa fa-facebook"></i></a></li>
																<li><a href="https://www.instagram.com/prefcarolina/" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/5.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://wa.me/5599981434997" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="https://www.instagram.com/costacomunicacaovisual/" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/6.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://wa.me/5599982251473" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="https://www.instagram.com/tatiane_nascimento_photo/" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/7.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://api.whatsapp.com/send/?phone=556334112300&text&type=phone_number&app_absent=0" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li><a href="https://www.instagram.com/toledofibra/?igshid=YmMyMTA2M2Y%3D" target="_blank"><i class="fa fa-instagram"></i></a></li>
															</ul>
														</div>
													</div>
												</div>

												<div class="col-md-6">
													<div class="mu-single-team">
														<div class="mu-single-team-img">
															<img src="assets/images/patrocinadores/8.webp" alt="img">
														</div>
														<div align="center" class="mu-single-team-content">
															<ul class="mu-team-social">
																<li><a href="https://wa.me/5599981159110" target="_blank"><i class="fa fa-whatsapp"></i></a></li>
																<li>hotelrilton@hotmail.com</li>
															</ul>
														</div>
													</div>
												</div>
												


											</div>
										</div>
									</div>
									
								</div>
							
						</div>
					</div>
				</div>
			</div>
		</section>
		<!-- End Portfolio -->
	</main>
	
	<!-- End main content -->	


 @include('site.layouts.footer')	